...
//  kxmovie
//
//  Created by QiuDong on 15/12/15.
//
//
...
#include "libavcodec/svq3.h"
#include "libavcodec/thread.h"
#include "libavcodec/vdpau_internal.h"

#include <assert.h>

const uint16_t ff_h264HD_mb_sizes[4] = { 256, 384, 512, 768 };

int avpriv_h264HD_has_num_reorder_frames(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
    return h ? h->sps.num_reorder_frames : 0;
}

static void h264HD_er_decode_mb(void *opaque, int ref, int mv_dir, int mv_type,
                              int (*mv)[2][4][2],
                              int mb_x, int mb_y, int mb_intra, int mb_skipped)
{
    ...
    ff_h264HD_hl_decode_mb(h);
}

void ff_h264HD_draw_horiz_band(H264Context *h, int y, int height)
{
 ...
 */
int ff_h264HD_check_intra4x4_pred_mode(H264Context *h)
{
    ...
    return 0;
} // FIXME cleanup like ff_h264HD_check_intra_pred_mode

/**
 * Check if the top & left blocks are available if needed and
 * change the dc mode so it only uses the available blocks.
 */
int ff_h264HD_check_intra_pred_mode(H264Context *h, int mode, int is_chroma)
{
...
const uint8_t *ff_h264HD_decode_nal(H264Context *h, const uint8_t *src,
...
void ff_h264HD_free_tables(H264Context *h, int free_rbsp)
{
    int i;
    H264Context *hx;
...
int ff_h264HD_alloc_tables(H264Context *h)
{
 ...
 */
int ff_h264HD_context_init(H264Context *h)
{
    ERContext *er = &h->er;
    ...
    if (CONFIG_ERROR_RESILIENCE) {
        ...
        er->avctx          = h->avctx;
        er->dsp            = &h->dsp;
        er->decode_mb      = h264HD_er_decode_mb;
        er->opaque         = h;
        er->quarter_sample = 1;
...
fail:
    return AVERROR(ENOMEM); // ff_h264HD_free_tables will clean up for us
}

static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size,
                            int parse_extradata);

int ff_h264HD_decode_extradata(H264Context *h, const uint8_t *buf, int size)
{
    AVCodecContext *avctx = h->avctx;
    int ret;
...
av_cold int ff_h264HD_decode_init(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
    int i;
    int ret;
    ...
    if (avctx->extradata_size > 0 && avctx->extradata) {
        ret = ff_h264HD_decode_extradata(h, avctx->extradata, avctx->extradata_size);
        if (ret < 0) {
            ff_h264HD_free_context(h);
            return ret;
        }
    }
...
void ff_h264HD_flush_change(H264Context *h)
{
    int i, j;
 ...
 */
int ff_h264HD_get_profile(SPS *sps)
{
...
int ff_h264HD_set_parameter_from_sps(H264Context *h)
{
...
static int get_last_needed_nal(H264Context *h, const uint8_t *buf, int buf_size)
{
    ...
    while(1) {
        ...
        ptr = ff_h264HD_decode_nal(h, buf + buf_index, &dst_length, &consumed,
                                 next_avc - buf_index);
...
static int h264HD_decode_frame(AVCodecContext *avctx, void *data,
                             int *got_frame, AVPacket *avpkt)
{
    ...
    if(h->is_avc && buf_size >= 9 && buf[0]==1 && buf[2]==0 && (buf[4]&0xFC)==0xFC && (buf[5]&0x1F) && buf[8]==0x67){
        ...
        return ff_h264HD_decode_extradata(h, buf, buf_size);
    }
not_extra:
...
av_cold void ff_h264HD_free_context(H264Context *h)
{
    int i;
    
    ff_h264HD_free_tables(h, 1); // FIXME cleanup init stuff perhaps
...
static av_cold int h264HD_decode_end(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
...
static const AVOption h264HD_options[] = {
    {"is_avc", "is avc", offsetof(H264Context, is_avc), FF_OPT_TYPE_INT, {.i64 = 0}, 0, 1, 0},
    {"nal_length_size", "nal_length_size", offsetof(H264Context, nal_length_size), FF_OPT_TYPE_INT, {.i64 = 0}, 0, 4, 0},
    {NULL}
};

static const AVClass h264HD_class = {
    .class_name = "H264 Decoder",
    .item_name  = av_default_item_name,
    .option     = h264HD_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

static const AVClass h264HD_vdpau_class = {
...
const int AV_CODEC_ID_H264_HD = AV_CODEC_ID_FFMETADATA + 1;

AVCodec ff_h264HD_decoder = {
    .name                  = "h264HD",
    .long_name             = NULL_IF_CONFIG_SMALL("H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10"),
    .type                  = AVMEDIA_TYPE_VIDEO,
    .id                    = AV_CODEC_ID_H264,//AV_CODEC_ID_H264_HD,
    .priv_data_size        = sizeof(H264Context),
    .init                  = ff_h264HD_decode_init,
    .close                 = h264HD_decode_end,
    .decode                = h264HD_decode_frame,
    ...
    .priv_class            = &h264HD_class,
};

#if CONFIG_H264_VDPAU_DECODER
AVCodec ff_h264HD_vdpau_decoder = {
    .name           = "h264HD_vdpau",
